...
export const stageContracts: StageContract[] = [
  ...
  // ── BLUEPRINT ──────────────────────────────────────────────────
  {
    ...
    hardGate: true,
    requiredSections: [
      { heading: 'Capability Map', required: true, minWordCount: 150 },
      { heading: 'Service Boundaries', required: true, minWordCount: 120, subsections: ['Data Ownership', 'API Surface'] },
      { heading: 'Dependency Graph', required: true, minWordCount: 80 },
      { heading: 'Migration Waves', required: true, minWordCount: 100 },
    ],
    requiredArtifacts: [
    ...
    requiredPatterns: [
      { name: 'mermaid_blocks', pattern: /```mermaid[\s\S]*?```/g, minOccurrences: 2, description: 'Must include Mermaid diagrams' },
    ],
  },
  ...
  // ── SPEC_LOCK ──────────────────────────────────────────────────
  {
    ...
    hardGate: true,
    requiredSections: [
      { heading: 'BDD Scenarios', required: true, minWordCount: 300, mustContain: ['Scenario', 'Given', 'When', 'Then'] },
      { heading: 'API Contracts', required: true, minWordCount: 100 },
      { heading: 'Acceptance Criteria', required: true, minWordCount: 100 },
      { heading: 'Edge Cases', required: true, minWordCount: 80 },
    ],
    requiredArtifacts: [
      { type: 'spec', description: 'Gherkin feature files', required: true, filePattern: '*.feature' },
    ],
    requiredPatterns: [
      { name: 'gherkin_scenarios', pattern: /Scenario[:\s]/g, minOccurrences: 5, description: 'Must have at least 5 BDD scenarios' },
      { name: 'given_when_then', pattern: /Given\s.+\n\s*When\s.+\n\s*Then\s/g, minOccurrences: 5, description: 'Must have complete Given/When/Then triplets' },
    ],
  },
  ...
  // ── ARCHITECT ──────────────────────────────────────────────────
  {
    ...
    hardGate: true,
    requiredSections: [
      { heading: 'Target Architecture', required: true, minWordCount: 150, subsections: ['Components', 'Technology Choices'] },
      { heading: 'Migration Strategy', required: true, minWordCount: 120, mustContain: ['phase', 'milestone'] },
      { heading: 'Risk Assessment', required: true, minWordCount: 100, mustContain: ['risk', 'mitigation'] },
      { heading: 'Infrastructure', required: true, minWordCount: 80 },
      { heading: 'Cost Estimate', required: true, minWordCount: 50 },
    ],
    requiredArtifacts: [
      { type: 'diagram', description: 'Target architecture diagram (Mermaid)', required: true },
    ],
    requiredPatterns: [
      { name: 'mermaid_blocks', pattern: /```mermaid[\s\S]*?```/g, minOccurrences: 1, description: 'Must include architecture diagram' },
      { name: 'alternatives_compared', pattern: /option|alternative|compare|vs\.|versus/gi, minOccurrences: 2, description: 'Must compare at least 2 approaches' },
    ],
  },
