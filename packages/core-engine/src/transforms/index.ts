...
/**
 * Add logging to function boundaries
 */
export function addBoundaryLogging(code: string, functionName: string): CodeTransformResult {
  const changes: string[] = [];
  const warnings: string[] = [];
  const suggestions: string[] = [];
  ...
  const functionPattern = new RegExp(
    `(public|private|protected)?\\s+(async\\s+)?function\\s+${functionName}\\s*\\([^)]*\\)\\s*{`,
  );

  if (!functionPattern.test(code)) {
    warnings.push(`Function ${functionName} not found`);
    return {
      originalCode: code,
  ...
  // Add exit logs (simplified - would need AST parsing for robustness)
  transformedCode = transformedCode.replace(
    /return\s+([^;]+);/g,
    'logger.debug("Exiting ' + functionName + '"); return $1;',
  );

  changes.push(`Added entry/exit logging to ${functionName}`);
  suggestions.push('Import logger at module level');
  suggestions.push('Consider using structured logging');
  suggestions.push('Add performance timing logs');
...
export function addErrorHandling(
  ...
  transformedCode = transformedCode.replace(
    functionPattern,
    (match, declaration, body, closing) => {
      const indent = '    ';
      return `${declaration}\n${indent}try {\n${body}\n${indent}} catch (error) {\n${indent}  logger.error('Error in ${functionName}:', error);\n${indent}  throw error;\n${indent}}${closing}`;
    },
  );

  changes.push(`Added try-catch to ${functionName}`);
  suggestions.push('Define specific error types to catch');
  suggestions.push('Add error recovery strategies');
  suggestions.push('Log relevant context with errors');
