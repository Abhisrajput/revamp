...
export const BreeOutputTab = memo(function BreeOutputTab({ stageName, pipelineRunId, className }: BreeOutputTabProps) {
  ...
  const { data: basicData, isLoading } = useQuery<any>({
    ...
    queryFn: async () => {
      ...
      const res = await getApiClient().get(`/pipeline/${pipelineRunId}/artifacts/${stageName}`);
      ...
      if (stageName !== 'SCAN') {
        const scanRes = await getApiClient().get(`/pipeline/${pipelineRunId}/artifacts/SCAN`);
  ...
  const { data: breeHealth } = useQuery<any>({
    ...
    queryFn: async () => { try { return (await getApiClient().get('/bree/readiness')).data; } catch { return null; } },
    staleTime: 60_000,
  });
  ...
  const deepAnalysis = useMutation({
    mutationFn: async () => {
      const res = await getApiClient().post('/bree/deep-analyze', {
